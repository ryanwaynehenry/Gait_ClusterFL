...
cluster_set = ['Participant-1', 'Participant-2']
variation_set = ['_left_hand', '_right_hand', '_left_pocket', '_right_pocket']
class_set = ['_neutral', '_limping_left_leg', '_limping_right_leg']
label = [0, 1, 1]

NUM_OF_CLASS = 2
DIMENSION_OF_FEATURE = 600
...
def load_data(user_id):
    ...
    if user_id < 4:
        cluster_id = 0
        intra_user_id = user_id
    else:
        cluster_id = 1
        intra_user_id = user_id - 5   # 5, 6, 7, 8, to 0, 1, 2, 3

    coll_class = []
    coll_label = []
    cluster_name = cluster_set[cluster_id]
    variation_name = variation_set[intra_user_id]
    for k in range(len(class_set)):
        read_path = 'C:\\Users\\ryanw\PycharmProjects\ClusterFL\datasets\walking_dataset\\' + \
            cluster_name + variation_name + class_set[k] + 'Merged.csv'
        ...
        temp_label = label[k] * np.ones(count_img)
...
def generate_data(num_of_train_per_node, num_of_test_per_node, x_coll, y_coll):
    ...
    x_train, x_test, y_train, y_test = \
        train_test_split(x_coll, y_coll, test_size=test_percent, random_state=64, shuffle=True)
